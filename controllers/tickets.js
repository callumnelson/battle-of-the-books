...
const deleteTicket = async (req, res) => {
  try {
    ...
    if (
      ticket.owner.equals(req.user.profile._id) ||
      req.user.profile.role > 100
    ) {
      ...
      throw new Error(
        `Access Denied: Students can't delete other students' tickets`
      );
    }
  } catch (err) {
    console.log(err);
...
const createApiTicket = async (req, res) => {
  try {
    ...
    if (req.user.profile.role > 100) {
      ...
      //Remove book from checked out books
      teacher.currentBooks.remove(book._id);
      await teacher.save();
      res.redirect("/tickets");
    } else {
      const book = await Book.findById(req.body.bookId);
...
const edit = async (req, res) => {
  try {
    ...
    if (
      req.user.profile.role < 200 &&
      !toEdit.owner.equals(req.user.profile._id)
    ) {
      throw new Error(
        `Access Denied: Students can't edit other students' tickets`
      );
    } else {
      const student = await Profile.findById(req.user.profile._id).populate({
        path: "tickets",
        populate: { path: "book" },
      });
...
const update = async (req, res) => {
  try {
    ...
    if (
      req.user.profile.role < 200 &&
      !toUpdate.owner.equals(req.user.profile._id)
    ) {
      throw new Error(
        `Access Denied: Students can't update other students' tickets`
      );
    } else {
